...
def generateIcfFromConf(confPath, driveInfo, managerName=socket.gethostname()):
	...
	'''
	Takes a CSV data dictionary and created an .icf file at the given path.  DriveInfo and optional manager name are
	...
	def generateIcfFromCsvLineData(csvData, icfFilePath, driveInfo, managerName=socket.gethostname()):
		...
		templateFilePath = "" # TODO: Get the path of the template file from quarchpy		
		csvTestName = None
		csvWorkers = None
		csvRequestSize = None
		csvReadPer = None
		...
		# Create the string modifications needed to set the correct target
		newStr = "\t" + str(driveInfo["PHYSICALDRIVE"]) +  ": \"" + str(driveInfo["NAME"]) + " " + str(driveInfo["FW_REV"]) + "\"" + "\n"
		# Loop through required workers and set their target
		for x in range(csvWorkers):
			oldStr = "[*TARGET_" + x + "*]"
			# Replace the string
			fileData = fileData.replace(str(oldStr),str(newStr))
			
		# Loop through required workers and set their access specification
		for x in range(csvWorkers):
			oldStr = "[*SPEC_" + x + "*]"
			# Replace the string
			fileData = accessSpecName		
...
Reds the data into a dictionary 
'''
def readIcfCsvLineData(csvFilePath, csvFileLine):
	# Open the file
	...
	for x in headerRow:
		if headerRow[x] = "TEST_NAME":
			csvDataLine[headerRow[x]] = dataRow[x]
		if headerRow[x] = "WORKER_COUNT":
			csvDataLine[headerRow[x]] = dataRow[x]
		if headerRow[x] = "REQUEST_SIZE":
			csvDataLine[headerRow[x]] = dataRow[x]
		if headerRow[x] = "READ_PER":
			csvDataLine[headerRow[x]] = dataRow[x]
		if headerRow[x] = "RANDOM_PER":
			csvDataLine[headerRow[x]] = dataRow[x]
		if headerRow[x] = "RUN_TIME":
			csvDataLine[headerRow[x]] = dataRow[x]
